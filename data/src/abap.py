...
def download_saba2_species(
    ...
    if not overwrite and os.path.exists(file_path):
        print(f"File for sabap2_id {sabap2_id} already exists. Skipping download.", flush=True)
        return
...
def combine(
    ...
    for file in tqdm(all_files, desc="Processing files"):
        ...
        # Group by pentad
        grouped = observations.groupby('pentad').agg({
            'observed': 'sum'  # Summing up the observations
        }).reset_index()
        ...
        reference_df = pd.merge(reference_df, grouped[['pentad', str(species_id)]], on='pentad', how='left')
        reference_df[str(species_id)] = reference_df[str(species_id)].fillna(0).astype(int)


    make_dir_if_not_exists(aggregate_dir)
    reference_df.to_csv(f"{aggregate_dir}/sabap2.csv", index=False)