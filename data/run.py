...
def generate_sabap_ebirds_diff():
    ...
    this has run the eBird_name column should be copied from the
...
def aggregate_ebirds():
    """Once the BirdList.csv file has been populated with the eBird_name
...
def download_bioclim():
    ...
    download_kaggle_dataset(
        config['KAGGLE_BIOCLIM_DATASET'], config['BIOCLIM_DIR']
    )
...
def download_google_ee_data():
    ...
    download_kaggle_dataset(
        config['KAGGLE_GOOGLE_EE_DATASET'], config['GOOGLE_EE_DIR']
    )
